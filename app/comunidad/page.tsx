...
function TavernHighlightsSkeleton() {
  return (
    <div className="rounded-xl border border-white/10 bg-black/40 p-1 backdrop-blur-xl animate-pulse w-full max-w-full sm:max-w-[380px] lg:ml-auto">
      <div className="rounded-lg bg-surface-dark/95 p-3 border border-white/5 overflow-hidden">
        {/* Header */}
        ...
        <div className="mt-2 pt-2 border-t border-white/10 space-y-2">
          ...
          <div className="flex gap-2 overflow-hidden -mx-1 px-1">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-[46px] w-[112px] shrink-0 bg-white/5 rounded-lg border border-white/5 flex items-center gap-2 p-1">
  ...
  return (
    <section className="container-page pt-8 sm:pt-10">
      <div className="tavern-panel p-4 sm:p-6 animate-pulse">
        <div className="h-4 w-24 bg-border-subtle rounded mb-2"></div>
        <div className="h-10 w-64 bg-border-subtle rounded"></div>
        <div className="scrollbar-hide -mx-4 sm:-mx-6 mt-6 flex w-[calc(100%+2rem)] sm:w-[calc(100%+3rem)] max-w-[calc(100%+2rem)] sm:max-w-[calc(100%+3rem)] gap-3 overflow-hidden px-4 sm:px-6 pb-3 sm:mx-0 sm:grid sm:w-auto sm:max-w-none sm:grid-cols-2 xl:grid-cols-4 sm:px-0 sm:pb-0 sm:overflow-visible">
          <div className="h-52 w-[85vw] min-w-[270px] max-w-[320px] sm:w-auto shrink-0 bg-paper/60 rounded-lg border border-border-subtle"></div>
          <div className="h-52 w-[85vw] min-w-[270px] max-w-[320px] sm:w-auto shrink-0 bg-paper/60 rounded-lg border border-border-subtle"></div>
          <div className="h-52 w-[85vw] min-w-[270px] max-w-[320px] sm:w-auto shrink-0 bg-paper/60 rounded-lg border border-border-subtle"></div>
          <div className="h-52 w-[85vw] min-w-[270px] max-w-[320px] sm:w-auto shrink-0 bg-paper/60 rounded-lg border border-border-subtle"></div>
        </div>
...
function TavernActivityFeedSkeleton() {
  return (
    <section className="tavern-panel p-4 sm:p-6 animate-pulse">
      <div className="h-4 w-24 bg-border-subtle rounded mb-2"></div>
      <div className="h-8 w-64 bg-border-subtle rounded mb-4"></div>
      <div className="h-11 w-full bg-border-subtle rounded mb-8"></div>
      
      <div className="space-y-6">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="flex gap-3">
            <div className="h-10 w-10 bg-border-subtle rounded-full shrink-0"></div>
  ...
  return (
    <section className="animate-pulse w-full">
      ...
      <div className="lg:hidden mb-6">
        <div className="h-4 w-36 bg-border-subtle rounded mb-3"></div>
        <div className="scrollbar-hide -mx-4 sm:-mx-6 flex w-[calc(100%+2rem)] sm:w-[calc(100%+3rem)] max-w-[calc(100%+2rem)] sm:max-w-[calc(100%+3rem)] gap-4 overflow-hidden px-4 sm:px-6 pb-2">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex flex-col items-center gap-1 shrink-0">
              <div className="h-14 w-14 rounded-full bg-border-subtle"></div>
              <div className="h-3 w-12 bg-border-subtle rounded mt-1"></div>
              <div className="h-2 w-8 bg-surface-muted/30 rounded mt-0.5"></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="hidden lg:block">
        <div className="h-4 w-24 bg-border-subtle rounded mb-2"></div>
        <div className="h-8 w-48 bg-border-subtle rounded mb-6"></div>
        <div className="grid gap-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-24 bg-surface-muted/30 rounded-lg border border-border-subtle"></div>
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section className="tavern-panel p-4 sm:p-6 lg:col-span-2 animate-pulse" aria-hidden="true">
      <div className="border-b border-border-subtle pb-4">
        ...
        <div className="h-7 w-56 bg-border-subtle rounded"></div>
      </div>

      {/* Mobile/Tablet Skeleton: Tabs + Active Tab Content (< lg) */}
      <div className="lg:hidden">
        {/* Segmented control track */}
        <div className="mt-4 flex gap-1 bg-surface-muted/30 p-1 rounded-lg">
          <div className="h-8 bg-white/70 rounded-md flex-1"></div>
          <div className="h-8 bg-surface-muted/30 rounded-md flex-1"></div>
          <div className="h-8 bg-surface-muted/30 rounded-md flex-1"></div>
        </div>
        {/* Content skeleton for first tab */}
        <div className="mt-4">
          <div className="h-4 w-44 bg-border-subtle rounded mb-3"></div>
          <div className="grid gap-2 sm:grid-cols-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-[76px] bg-surface-muted/30 rounded-lg border border-border-subtle"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:grid lg:grid-cols-[1.4fr_0.8fr_0.8fr] lg:gap-6 lg:mt-6">
        ...
        <div>
          <div className="h-4 w-32 bg-border-subtle rounded mb-3"></div>
          <div className="space-y-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-[60px] bg-surface-muted/30 rounded-lg border border-border-subtle"></div>
            ))}
          </div>
        </div>
        <div>
...
function TavernHighlights({ overview }: { overview: TavernOverview }) {
  ...
  return (
    <div className="group relative w-full min-w-0 max-w-full overflow-hidden rounded-xl border border-white/10 bg-black/40 p-1 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:border-white/20 hover:bg-black/50 sm:max-w-[380px] lg:ml-auto">
      ...
      <div className="hidden md:block absolute -inset-[100%] z-0 animate-[spin_10s_linear_infinite] bg-gradient-to-r from-transparent via-accent/20 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
      
      <div className="relative z-10 rounded-lg bg-surface-dark/95 p-3 backdrop-blur-md border border-white/5 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 pb-2">
          <div className="flex min-w-0 items-center gap-2.5">
            <div className="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent/20 ring-1 ring-accent/30">
              <div className="absolute inset-0 rounded-full animate-ping bg-accent/30" />
              <Activity className="text-accent relative z-10" size={14} />
            </div>
            <div className="min-w-0">
              <h2 className="max-w-full break-words font-display text-[11px] font-bold uppercase leading-tight tracking-[0.06em] text-text-on-dark/90 sm:text-xs sm:tracking-[0.1em]">
                El pulso de la comunidad
              </h2>
              <p className="text-[10px] font-bold text-text-on-dark-muted/50 uppercase tracking-wider mt-0.5">Últimos 7 días</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          {/* Movimientos */}
          <Link href="#actividad" className="relative overflow-hidden rounded-lg border border-white/5 bg-white/5 p-2 text-center transition duration-300 hover:bg-white/10 hover:border-accent/30 group/stat">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center">
              <Activity size={13} className="text-text-on-dark-muted/40 mb-1 group-hover/stat:text-text-on-dark-muted/80 transition-colors" />
              <p className="font-display text-lg sm:text-xl font-bold leading-none text-white">{highlights.weeklyActivityCount}</p>
              <p className="mt-1 text-[9px] font-bold uppercase tracking-wider text-text-on-dark-muted/50">Movimientos</p>
            </div>
          </Link>

          {/* Añadidos */}
          <Link href="#ultimos-juegos" className="relative overflow-hidden rounded-lg border border-white/5 bg-white/5 p-2 text-center transition duration-300 hover:bg-white/10 hover:border-accent/30 group/stat">
            <div className="absolute -right-4 -bottom-4 opacity-[0.02] group-hover/stat:opacity-10 transition-opacity duration-500 group-hover/stat:scale-110">
              <Dices size={48} />
            </div>
            <div className="relative z-10 flex flex-col items-center justify-center">
              <Dices size={13} className="text-text-on-dark-muted/40 mb-1 group-hover/stat:text-text-on-dark-muted/80 transition-colors" />
              <p className="font-display text-lg sm:text-xl font-bold leading-none text-white">{highlights.weeklyLibraryAdds}</p>
              <p className="mt-1 text-[9px] font-bold uppercase tracking-wider text-text-on-dark-muted/50">Añadidos</p>
            </div>
          </Link>

          {/* Deseos */}
          <Link href="#juegos-mas-queridos" className="relative overflow-hidden rounded-lg border border-white/5 bg-white/5 p-2 text-center transition duration-300 hover:bg-white/10 hover:border-accent/30 group/stat">
            <div className="absolute -right-4 -bottom-4 opacity-[0.02] group-hover/stat:opacity-10 transition-opacity duration-500 group-hover/stat:scale-110">
              <Heart size={48} />
            </div>
            <div className="relative z-10 flex flex-col items-center justify-center">
              <Heart size={13} className="text-text-on-dark-muted/40 mb-1 group-hover/stat:text-accent transition-colors" />
              <p className="font-display text-lg sm:text-xl font-bold leading-none text-white">{highlights.topWantedGame?.count || 0}</p>
              <p className="mt-1 text-[9px] font-bold uppercase tracking-wider text-text-on-dark-muted/50">Deseos</p>
            </div>
          </Link>
        </div>
        ...
        {mostWanted && mostWanted.length > 0 && (
          <div className="mt-2 pt-2 border-t border-white/10">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-bold uppercase tracking-wider text-text-on-dark-muted/60 flex items-center gap-1.5">
                <Flame size={12} className="text-accent" /> Tendencias actuales
              </p>
              <Link href="#juegos-mas-queridos" className="text-[10px] font-bold uppercase tracking-wider text-text-on-dark-muted/40 hover:text-accent transition-colors flex items-center gap-0.5">
              ...
              {mostWanted.slice(0, 3).map((game) => {
                return (
                  <Link 
                    ...
                    prefetch={false}
                    className="group/game relative grid w-[112px] flex-none grid-cols-[38px_minmax(0,1fr)] items-center gap-2 rounded-md border border-white/5 bg-black/40 p-1.5 transition duration-300 hover:bg-white/10 hover:border-accent/30 snap-start"
                  >
                    ...
                    <div className="min-w-0">
                      <h3 className="truncate text-[10px] font-bold leading-tight text-white group-hover/game:text-accent transition-colors">
                        {game.title}
                      </h3>
                      <p className="mt-0.5 flex items-center gap-1 text-[9px] font-semibold text-text-on-dark-muted/65">
                        <Heart size={9} className="fill-accent text-accent" /> {game.count} {game.count === 1 ? 'deseo' : 'deseos'}
                      </p>
                    </div>
                  </Link>
                );
