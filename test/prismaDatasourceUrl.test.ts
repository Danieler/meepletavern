...
test("production keeps the pooler and lets the dedicated limit override URL parameters", () => {
  const result = getRuntimeDatasourceUrl({
    nodeEnv: "production",
...
test("production falls back to one connection when a URL contains an unsafe limit", () => {
  const result = getRuntimeDatasourceUrl({
    nodeEnv: "production",
    databaseUrl: "postgresql://user:pass@pool.example.test:6543/app?pgbouncer=true&connection_limit=21"
  });
