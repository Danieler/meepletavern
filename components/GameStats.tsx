...
export function GameStats({ game }: { game: CatalogGame }) {
  ...
  return (
    <dl className="grid grid-cols-1 gap-2 min-[360px]:grid-cols-2 2xl:grid-cols-4">
      {stats.map((stat) => (
        <div key={stat.label} className="flex min-h-16 min-w-0 items-center gap-3 rounded-md border border-walnut/10 bg-white/75 px-3 py-2 shadow-soft">
          ...
          <div className="min-w-0">
            ...
            <dd className="mt-0.5 break-words text-sm font-extrabold leading-tight text-wood min-[400px]:text-base">{stat.value}</dd>
          </div>
        </div>
      ))}
    </dl>
